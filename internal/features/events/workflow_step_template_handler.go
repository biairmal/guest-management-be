...
// WorkflowStepTemplateHandler exposes HTTP handlers for category-scoped workflow step template CRUD.
type WorkflowStepTemplateHandler struct {
	service   WorkflowStepTemplateService
	validator validation.Validator
}

// workflowStepTemplateListConfig declares the allow-listed sort/filter
// fields for workflow step template list queries. category_id is always
// scoped from the URL, not a query filter. Pagination falls back to the
// shared defaults.
...
// NewWorkflowStepTemplateHandler returns a WorkflowStepTemplateHandler that
// uses the given service and validator. Both are interfaces, allowing easy
// testing and substitution.
func NewWorkflowStepTemplateHandler(
	service WorkflowStepTemplateService, validator validation.Validator,
) *WorkflowStepTemplateHandler {
	return &WorkflowStepTemplateHandler{service: service, validator: validator}
}
...
//	@Success		200			{object}	common.PageResponse[events.WorkflowStepTemplate]
...
func (h *WorkflowStepTemplateHandler) List(r *http.Request) (any, error) {
...
//	@Success		200			{object}	events.WorkflowStepTemplate
...
func (h *WorkflowStepTemplateHandler) GetByID(r *http.Request) (any, error) {
	categoryID, err := categoryIDFromPath(r)
	if err != nil {
		return nil, err
	}
...
//	@Produce		json
//	@Param			category_id	path		string									true	"Event category UUID"
//	@Param			body		body		events.CreateWorkflowStepTemplateInput	true	"Workflow step template payload"
//	@Success		201			{object}	events.WorkflowStepTemplate
...
//
//nolint:dupl // decode+validate+create+respond shape intentionally mirrors WorkflowStepHandler.Create (see PATTERNS.md)
func (h *WorkflowStepTemplateHandler) Create(r *http.Request) (any, error) {
	categoryID, err := categoryIDFromPath(r)
	if err != nil {
		return nil, err
	}
...
//	@Param			category_id	path		string									true	"Event category UUID"
//	@Param			id			path		string									true	"Workflow step template UUID"
//	@Param			body		body		events.UpdateWorkflowStepTemplateInput	true	"Fields to update"
//	@Success		200			{object}	events.WorkflowStepTemplate
...
func (h *WorkflowStepTemplateHandler) Update(r *http.Request) (any, error) {
	categoryID, err := categoryIDFromPath(r)
	if err != nil {
		return nil, err
	}
...
func (h *WorkflowStepTemplateHandler) Delete(r *http.Request) (any, error) {
	categoryID, err := categoryIDFromPath(r)
	if err != nil {
		return nil, err
	}
